...
readonly TITLE="🚀 T3 Gemstone Board Examples"
...
readonly DESCRIPTION=$(
  ...
  Powered by Devbox, uv, ${BOLD}Love${NO_COLOR} and ${BOLD}Passion${NO_COLOR}.
EOF
)
...
check_requirements() {
    ...
    for i in "curl" "git" "sudo"; do
        if ! has "$i"; then
            echo "$i must be installed to start the installation! For Help: $DOCS_URL/sdk"
            fail_task
            return 1
        fi
    done
